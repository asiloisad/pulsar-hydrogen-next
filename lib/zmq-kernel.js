...
export default class ZMQKernel extends KernelTransport {
  ...
  async connect(done) {
    ...
    await this.shellSocket.connect(address + this.connection.shell_port)
    await this.ioSocket.connect(address + this.connection.iopub_port)
    await this.ioSocket.subscribe("")
    await this.stdinSocket.connect(address + this.connection.stdin_port)
