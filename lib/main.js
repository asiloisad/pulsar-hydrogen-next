...
function runAllInline() {
  ...
  checkForKernel(store, async kernel => {
    ...
    const currPos = editor.getCursorBufferPosition()

    // Start from beginning (without scrolling)
    ...
    while (true) {
      ...
      if (!codeBlock || codeBlock.code === null) {
        // Move to next line if no code block found
        const currRow = editor.getCursorBufferPosition().row
        if (currRow >= lastRow) break
        editor.setCursorBufferPosition([currRow + 1, 0], { autoscroll: false })
        continue
      }
      ...
      // Wait for this block to complete before moving to next
      const success = await result.createResultAsync(store, { code: processedCode, row, cellType })
      ...
      // Move down after execution (find next non-blank row)
      let nextRow = row + 1
      while (nextRow < lastRow && codeManager.isBlank(editor, nextRow)) {
        nextRow++
      }
...
function runAllAboveInline() {
  ...
  checkForKernel(store, async kernel => {
    ...
    while (true) {
      ...
      // Move down after execution (find next non-blank row)
      let nextRow = row + 1
      while (nextRow <= targetRow && codeManager.isBlank(editor, nextRow)) {
        nextRow++
      }

      // Stop if we've passed the target row
      if (nextRow > targetRow) break
...
function runAllBelowInline() {
  ...
  checkForKernel(store, async kernel => {
    ...
    // Start from current position (without scrolling)
    while (true) {
      const selection = editor.getLastSelection()
      const codeBlock = codeManager.findCodeBlock(editor, selection)
      const currRow = editor.getCursorBufferPosition().row
