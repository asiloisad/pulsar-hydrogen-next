...
export class KernelManager {
  ...
  async getKernelSpecForGrammar(grammar, editor) {
    ...
    // Check for magic comment "#:: kernelname" in first line
    if (editor) {
    ...
    if (
      ...
      atom.config.get("hydrogen-next.autoKernelPicker")
    ) {
  ...
  /**
   * Check for magic comment "#:: kernelname" in first line of editor.
  ...
  _getKernelFromMagicComment(editor, kernelSpecs) {
    ...
    const found = kernelSpecs.find(
      (spec) =>
        spec.name === requestedKernel ||
        spec.display_name === requestedKernel
    );
