...
export function findCodeBlockAtRow(editor, row) {
  ...
  const pyblock = getPythonCode(editor, row);
  if (pyblock) {
    return { code: pyblock.code, row: pyblock.endRow };
  }
...
function getPythonCode(editor, startRow) {
  if (editor.tokenizedBuffer.grammar.scopeName !== "source.python") {
    return;
  }

  // Check if current line is a function or class definition
  const currentLine = editor.lineTextForBufferRow(startRow);
  let isFunctionOrClass = currentLine.match(/^ *(def|class|async +def) +\w+/);
  let functionRow = startRow; // The row of the actual def/class line

  // Check if current line is a decorator - if so, find the function/class it decorates
  if (!isFunctionOrClass && currentLine.match(/^ *@\w+/)) {
    // Look forward to find the function/class this decorator applies to
    const lineCount = editor.getLineCount();
    for (let i = startRow + 1; i < lineCount; i++) {
      const line = editor.lineTextForBufferRow(i);
      // Skip blank lines and other decorators
      if (line.trim().length === 0 || line.match(/^ *@\w+/)) {
        continue;
      }
      // Check if this is a function or class definition
      if (line.match(/^ *(def|class|async +def) +\w+/)) {
        // Found it - use this row as the function/class row
  ...
  // If it's a function or class (or we're on a decorator for one), look backwards for decorators
  if (isFunctionOrClass) {
    let decoratorStartRow = startRow;
    // Look backwards for decorators (lines starting with @)
    ...
    // Now find the end of the function/class (use functionRow for indentation)
    const base = editor.indentationForBufferRow(functionRow);
    const lineCount = editor.getLineCount();
    let endRow = functionRow;

    let nonEmpty = functionRow;
    for (let i = functionRow + 1; i < lineCount; i++) {
    ...
    return { code: normalizeString(code), startRow: decoratorStartRow, endRow: nonEmpty };
  }

  // extend if-elif-else and try-except-else-finally
  // cursor must be placed at if|try line
  if (currentLine.match(/^ *(if|try).*:.*$/)) {
    let base = editor.indentationForBufferRow(startRow);
    let lineCount = editor.getLineCount();
    let endRow = startRow;
    for (let i = startRow + 1; i < lineCount; i++) {
      let text = editor.lineTextForBufferRow(i);
      if (text.length === 0) {
        continue;
      }
      let ilvl = editor.indentationForBufferRow(i);
      if (base < ilvl) {
        endRow = i;
        continue;
      } else if (ilvl < base) {
        break;
      } else if (text.match(/^ *(elif|else|except|finally).*:.*$/)) {
        endRow = i;
        continue;
      } else {
        break;
