...
export function activate() {
  ...
  store.subscriptions.add(
    atom.commands.add("atom-text-editor:not([mini])", {
      ...
      "hydrogen-next:clear-results": () => require("./result").clearResults(store),
      ...
      "hydrogen-next:previous-cell": () => require("./cell-navi").previousCell(),
...
function attachResultToClaude() {
  ...
  claudeChatService.setAttachContext({
    type: "selection",
    paths: [filePath || kernel.language || "output"],
    line: 1,
    selection: content,
