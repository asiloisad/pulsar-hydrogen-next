...
function extractVegaSpec(html) {
  ...
  while ((match = iifePattern.exec(html)) !== null) {
    ...
    if (specStr) {
      try {
        ...
        if (spec.$schema || spec.mark || spec.data || spec.layer || spec.vconcat || spec.hconcat || spec.config) {
          return spec
        }
      } catch (e) {
    ...
    if (commaIndex !== -1) {
      ...
      if (braceIndex !== -1) {
        ...
        if (specStr) {
          try {
            const spec = JSON.parse(specStr)
            if (spec.$schema || spec.mark || spec.data || spec.layer || spec.vconcat || spec.hconcat) {
              return spec
            }
          } catch (e) {
            // JSON parse failed
  ...
  const jsonScriptMatch = html.match(/<script[^>]*type\s*=\s*["']application\/json["'][^>]*>([\s\S]*?)<\/script>/i)
  if (jsonScriptMatch) {
    try {
  ...
  if (specVarMatch) {
    ...
    if (braceIndex !== -1 && braceIndex - startIndex < 10) { // Allow some whitespace
  ...
  const schemaMatch = html.match(/\{\s*"[^"]*"\s*:\s*\{[^}]*\}[^}]*"\$schema"\s*:\s*"[^"]*vega[^"]*"/)
  if (schemaMatch) {
...
function isVegaHTML(html) {
  return html.includes("vegaEmbed") ||
...
export class HTML extends React.PureComponent {
  ...
  processHTML(html) {
    ...
    this.setState({ isVega: false, vegaSpec: null, vegaMediaType: null })
  ...
  render() {
    ...
    // Render container for non-Vega HTML
    return (
      <div
        ref={this.elRef}
