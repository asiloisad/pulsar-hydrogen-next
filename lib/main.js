...
export function activate() {
  ...
  store.subscriptions.add(
    ...
    atom.commands.add("atom-workspace", {
      ...
      "hydrogen-next:show-inspector": () => require("./commands").showInspector(store),
...
function handleAdapterKernelSignal(command, event = null) {
  const handled = require("./hydrogen-adapter-integration").handleAdapterKernelCommand(
...
function runAdapterCommand(scope, moveDown = false, event = null) {
  const handled = require("./hydrogen-adapter-integration").runAdapterTargets(
    hydrogenAdapterServices,
    kernelManager,
    { scope, moveDown },
  );
...
function clearAdapterResults(event = null) {
  const handled = require("./hydrogen-adapter-integration").clearAdapterResults(
    hydrogenAdapterServices,
  );
  if (handled) event?.stopPropagation?.();
  return handled;
