...
export default class ZMQKernel extends KernelTransport {
  ...
  async connect(done) {
    if (!jmp) { jmp = await import("jmp") } // load if needed
  ...
  onShellMessage(message) {
    ...
    if (callback) {
      // Fix kernel status issue; error is caused by the fact that the IO message sometimes comes after the Shell message. This poses a problem in the workflow of the programme. The `setTimeout` delay used as temporary solution.
      setTimeout(() => { callback(message, "shell") })
    }
  }
