...
export default class ExecPanel {
   ...
   * @param {Function} onCancelled - callback when input is cancelled
   */
  showInput({ prompt, defaultText, allowCancel = true, password = false }, onConfirmed, onCancelled) {
    this.mode = "input";
  ...
  createElement() {
    ...
    this.disposables.add(
      atom.commands.add(this.editor.element, {
        ...
        "core:cancel": () => {
          if (this.allowCancel) {
            const callback = this.onCancelled; // Save before hide() clears it
            this.hide();
