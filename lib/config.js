...
const Config = {
  ...
  getGatewaysPath() {
    return path.join(atom.getConfigDirPath(), "gateways.cson");
  },
  ...
  getGateways(_default = []) {
    ...
    try {
      const gateways = CSON.parse(fs.readFileSync(gatewaysPath, "utf8"));
      if (gateways instanceof Error) {
        throw gateways;
      }
      if (!Array.isArray(gateways)) {
        throw new Error("Expected gateways.cson to contain an array of gateway objects");
      }
  ...
  ensureGatewaysFile(gatewaysPath) {
    ...
    if (oldValue && typeof oldValue === "string") {
      try {
        contents = CSON.stringify(JSON.parse(oldValue));
  ...
  schema: {
    ...
    autocompleteSuggestionPriority: {
      ...
      description:
        "Specify the sort order of hydrogen-next's autocomplete suggestions. Note the default providers like snippets have priority of `1`.",
      type: "integer",
      default: 3,
    },
    importNotebookURI: {
    ...
    jupyterCommand: {
      ...
      description:
        "Command used by `Launch Jupyter Console` and `Copy Jupyter Console Command`. Use `{connection-file}` as a placeholder for the active kernel's connection file path (it will be quoted automatically).",
      type: "string",
      default: "jupyter console --existing {connection-file}",
    },
  },
};
