...
// Guard against concurrent inline execution loops
let _inlineExecutionInProgress = false;
...
function runAllInline() {
  ...
  if (_inlineExecutionInProgress) {
    atom.notifications.addWarning("Inline execution already in progress");
    return;
  }
  checkForKernel(store, async (kernel) => {
    ...
    _inlineExecutionInProgress = true;
    atom.commands.dispatch(editor.element, "autocomplete-plus:cancel");
    const codeManager = require("./code-manager");
    const result = require("./result");
    ...
    try {
      ...
      _inlineExecutionInProgress = false;
    }
  });
}
...
function recalculateAllInline() {
  ...
  clearAndCenter();
  restartKernel();
  // Reset flag since kernel is restarted and any pending execution is invalidated
  _inlineExecutionInProgress = false;
  runAllInline();
}
