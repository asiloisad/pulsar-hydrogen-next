...
export default class ZMQKernel extends KernelTransport {
  ...
  onIOMessage(message) {
    ...
    if (msg_type === "status") {
      ...
      // Flush deferred shell reply BEFORE updating execution state to avoid race condition:
      // If we set executionState="idle" first, ResultView sees status="running" + kernel idle
      // and incorrectly shows error. By flushing first, the shell callback sets OutputStore.status
      // before the UI re-renders due to executionState change.
      if (status === "idle") {
