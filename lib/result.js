...
export function clearResults({ editor, kernel, markers }) {
  ...
  // Emit event for other packages (e.g., scroll-keeper) to handle position recovery
  if (editor && editor.emitter) {
    editor.emitter.emit("hydrogen-cleared");
  }
}
      ...
      switch (cellType) {
        ...
        case "codecell":
          ...
          const tryResolve = () => {
            ...
            // Small delay needed to allow kernel to stabilize before next request
            if (shellStatus !== null && kernelIdle) {
              setTimeout(() => resolve(shellStatus === "ok"), 10);
            }
          };
