...
function extractSpacingFromText(text) {
  ...
  const spacingPatterns = [
    ...
    { pattern: /\\\\(?:\[(\d+(?:\.\d+)?)(pt|em|ex|cm|mm|in)?\])?/g, getHeight: (m) => m[1] ? latexLengthToCSS(m[1], m[2] || 'pt') : '1em' },
    // \vspace{10pt}
    { pattern: /\\vspace\*?\s*\{(\d+(?:\.\d+)?)(pt|em|ex|cm|mm|in)?\}/g, getHeight: (m) => latexLengthToCSS(m[1], m[2] || 'pt') },
    // \addvspace{\medskipamount} etc
    { pattern: /\\addvspace\s*\{\\(medskipamount|bigskipamount|smallskipamount)\}/g, getHeight: (m) => {
      switch (m[1]) {
        case 'smallskipamount': return '0.25em'
        case 'medskipamount': return '0.5em'
        case 'bigskipamount': return '1em'
        default: return '0.5em'
      }
    }},
    // \addvspace{10pt}
    { pattern: /\\addvspace\s*\{(\d+(?:\.\d+)?)(pt|em|ex|cm|mm|in)?\}/g, getHeight: (m) => latexLengthToCSS(m[1], m[2] || 'pt') },
    // \smallskip, \medskip, \bigskip
    { pattern: /\\(smallskip|medskip|bigskip)\b/g, getHeight: (m) => {
      switch (m[1]) {
        case 'smallskip': return '0.25em'
        case 'medskip': return '0.5em'
        case 'bigskip': return '1em'
        default: return '0.5em'
      }
    }},
    // \par - paragraph break
  ...
  for (const s of filtered) {
    if (s.start > pos) {
      const textPart = remaining.slice(pos, s.start)
  ...
  if (pos < remaining.length) {
    const textPart = remaining.slice(pos)
    const cleaned = cleanLatexForPlainText(textPart)
    if (cleaned) {
      result.push({ type: 'text', content: cleaned })
    }
...
function extractMathBlocks(latex) {
  ...
  const envNames = ['equation\\*?', 'align\\*?', 'gather\\*?', 'multline\\*?', 'eqnarray\\*?']
  ...
  while ((match = inlineDollarPattern.exec(latex)) !== null) {
    ...
    if (latex[match.index - 1] !== '$' && latex[match.index + match[0].length] !== '$') {
      mathBlocks.push({
        start: match.index,
        end: match.index + match[0].length,
        content: match[0],
  ...
  for (const block of filteredBlocks) {
    ...
    // Add math block
    blocks.push({ type: 'math', content: block.content, displayMode: block.displayMode })
    pos = block.end
  }
...
function stripDelimiters(latex) {
  ...
  const envMatch = stripped.match(/\\begin\{(equation\*?|align\*?|gather\*?|multline\*?)\}([\s\S]*)\\end\{\1\}/)
  if (envMatch) {
...
export class LaTeX extends React.Component {
  ...
  renderLatex() {
    ...
    try {
      ...
      if (blocks.length === 0 || blocks.every(b => b.type === 'text' || b.type === 'spacer')) {
        this.setState({
          renderedBlocks: [{ type: 'text', content: cleanLatexForPlainText(latex) }],
          error: null
        })
        return
      }
    ...
    if (this.state.renderedBlocks) {
      return (
        <div className="latex-display">
          {this.state.renderedBlocks.map((block, i) => {
            if (block.type === 'text') {
              if (!block.content) return null
              return (
                <div key={i} className="latex-text" style={{
                  whiteSpace: 'pre-wrap',
                  marginBottom: '0.5em'
                }}>
                  {block.content}
              ...
              return (
                <div
                  key={i}
                  className={block.displayMode ? "latex-math-display" : "latex-math-inline"}
                  style={block.displayMode ? { textAlign: 'center', margin: '0.5em 0' } : { display: 'inline' }}
