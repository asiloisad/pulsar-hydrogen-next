...
} from "../shared/ansi-utils";
import {
  reduceOutputs as sharedReduceOutputs,
  isSingleLine as sharedIsSingleLine,
  OUTPUT_TYPES,
} from "../shared/output-utils";

// Re-export for backward compatibility
export { escapeCarriageReturn, escapeCarriageReturnSafe };
...
export function reduceOutputs(outputs, output) {
  return sharedReduceOutputs(outputs, output);
}
...
export function isSingleLine(text, availableSpace) {
  return sharedIsSingleLine(text, availableSpace);
}
...
export default class OutputStore {
  ...
  @observable
  truncated = false; // Indicates if output was truncated due to backpressure
  @observable
  totalOutputCount = 0; // Track total outputs received (even if truncated)
