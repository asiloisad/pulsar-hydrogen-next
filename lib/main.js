...
import * as cellsNavi from "./cells-navi"
import {
  log,
...
export function activate() {
  ...
  store.subscriptions.add(
    atom.commands.add("atom-text-editor:not([mini])", {
      ...
      'hydrogen-next:next-cell': () => cellsNavi.nextCell(),
      'hydrogen-next:previous-cell': () => cellsNavi.previousCell(),
      'hydrogen-next:select-cell': () => cellsNavi.selectCell(),
      'hydrogen-next:select-up': () => cellsNavi.selectUp(),
      'hydrogen-next:select-down': () => cellsNavi.selectDown(),
      'hydrogen-next:move-cell-up': () => cellsNavi.moveCellUp(),
      'hydrogen-next:move-cell-down': () => cellsNavi.moveCellDown(),
    })
  )
  store.subscriptions.add(
    atom.commands.add("atom-workspace", {
      ...
      "hydrogen-next:show-inspector": () => commands.showInspector(store),
      "hydrogen-next:hide-inspector": () => commands.hideInspector(store),
    })
  )
...
function run(moveDown = false) {
  ...
  for (let codeBlock of codeManager.findCodeBlocks(editor)) {
    if (!codeBlock) { return }
