 ...
 */
export function RichMedia({ data, metadata, children }) {
  ...
  for (const child of childArray) {
    if (React.isValidElement(child)) {
      const mediaType =
        child.props.mediaType || child.type.defaultProps?.mediaType || child.type.MIMETYPE;
      if (mediaType) {
  ...
  for (const mimeType of MIME_PRIORITY) {
    if (data[mimeType] !== undefined && handlers.has(mimeType)) {
      ...
      return React.cloneElement(handler, { data: data[mimeType], metadata: mimeMetadata });
    }
  }
