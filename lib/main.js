...
export function consumeScrollMap(scrollMapService) {
  ...
  class HydrogenLayer extends scrollMapService.ScrollMapLayer {
    constructor(editor) {
      super({ editor: editor, name: "hydrogen", timer: 50 });
      this.threshold = 0;
      this.disposables.add(
        this.editor.onDidStopChanging(() => this.update()),
        atom.config.observe("hydrogen-next.scrollMapThreshold", (value) => {
    ...
    recalculate() {
      ...
      const breakpoints = codeManager.getBreakpoints(this.editor);
      ...
      if (this.threshold && this.threshold < validBreakpoints.length) {
        return;
      }
  ...
  const updateLayer = () => {
    if (atom.config.get("hydrogen-next.scrollMapState")) {
      scrollMapService.registerLayer("hydrogen", HydrogenLayer);
    } else {
