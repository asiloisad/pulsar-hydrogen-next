...
export function findCodeBlock(editor, selection) {
  if (!selection.isEmpty()) {
    ...
    if (selectedRange.end.column === 0) { endRow -= 1 }
    endRow = escapeBlankRows(editor, startPoint.row, endRow)
    return { code: getTextInRange(editor, startPoint, selectedRange.end), row: endRow }
  } else {
...
function getPythonCode(editor, startRow) {
  ...
  const isFunctionOrClass = currentLine.match(/^ *(def|class|async +def) +\w+/)

  // If it's a function or class, look backwards for decorators
  if (isFunctionOrClass) {
    ...
    // Now find the end of the function/class
    const base = editor.indentationForBufferRow(startRow)
    ...
    const code = editor.getTextInBufferRange([[decoratorStartRow, 0], [endRow + 1, 0]])
    return { code: normalizeString(code), startRow: decoratorStartRow, endRow }
  }
  ...
  if (currentLine.match(/^ *(if|try).*:.*$/)) {
    ...
    const code = editor.getTextInBufferRange([[startRow, 0], [endRow+1, 0]])
...
export function prepareCellDecoration(editor) {
  ...
  editor.decorateMarkerLayer(editor.buffer.hbpMarkerLayer, {
    type:'line', class: 'hydrogen-next-breakpoint'
  })
}

export function updateCellMarkers(editor) {
  const buffer = editor.buffer
  if (!buffer.hbpMarkerLayer) { return }
  buffer.hbpMarkerLayer.clear()
  const breakpoints = getBreakpoints(editor).slice(0,-1)
  for (const breakpoint of breakpoints) {
    buffer.hbpMarkerLayer.markRange([
      breakpoint, [breakpoint.row, 1e9]
    ], { invalidate:'surround', exclusive:true })
  }
}
