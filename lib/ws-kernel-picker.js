...
export default class WSKernelPicker {
  ...
  async toggle(_kernelSpecFilter) {
    ...
    if (!gateways.length) {
      atom.notifications.addError("No remote kernel gateways available", {
        description:
          "Use Hydrogen Next: Open Gateways Config to edit gateways.cson. Hydrogen can use remote kernels on either a Jupyter Kernel Gateway or Jupyter notebook server.",
      });
      return;
    }
  ...
  async onGateway(gatewayInfo) {
    ...
    try {
      ...
      // Handle auth errors or unknown errors - show error and close
