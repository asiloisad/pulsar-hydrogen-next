...
import { msgSpecToNotebookFormat, OUTPUT_TYPES } from "./shared/output-utils";
...
export default class ZMQKernel extends KernelTransport {
  ...
  monitor(done, prev) {
    try {
      ...
      const monitor = (socketName, socket) => {
        log(`ZMQKernel: monitor ${socketName}, isRestart=${!!prev}, isConnected=${socket.isConnected}`);
  ...
  _executeStartupCode() {
    ...
    if (this.language === "python") {
      const autoreloadMode = atom.config.get("hydrogen-next.pythonAutoreload");
      if (autoreloadMode !== "off") {
        log("KernelManager: Loading Python autoreload extension");
        this.execute("%load_ext autoreload\n", () => {});
        const printActivity = atom.config.get("hydrogen-next.pythonAutoreloadPrint");
        const modeCommand = printActivity
          ? `%autoreload ${autoreloadMode} --print`
          : `%autoreload ${autoreloadMode}`;
        this.execute(modeCommand + "\n", () => {});
        log("KernelManager: Autoreload configured:", autoreloadMode, printActivity ? "(with logging)" : "");
      }
    }
  }
  ...
  onIOMessage(message) {
    ...
    } else if (msg_id && OUTPUT_TYPES.includes(msg_type)) {
