...
/**
 * Reactive wrapper: reads the observable payload / selectedRow and feeds them to
 * the canvas grid as plain props (so the grid's lifecycle can diff them).
 * Non-tabular kinds (scalars) fall back to a plain text representation.
 */
const DataExplorerGrid = observer(({ des }) => {
  ...
  if (!Array.isArray(payload.rows) || !Array.isArray(payload.columns)) {
    return (
      <div className="data-explorer-scalar native-key-bindings">
